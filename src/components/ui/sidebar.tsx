...
function getSidebarCookieState(defaultValue = true): boolean {
...
>(({ side = 'left', variant = 'sidebar', collapsible = 'offcanvas', className, children, ...props }, ref) => {
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          `relative h-svh w-[--sidebar-width] bg-transparent transition-[width] duration-${SIDEBAR_ANIMATION_DURATION} ease-linear`,
      ...
      <div
        className={cn(
          `fixed inset-y-0 z-10 hidden h-svh w-[--sidebar-width] transition-[left,right,width] duration-${SIDEBAR_ANIMATION_DURATION} ease-linear md:flex`,
          side === 'left'
...
const SidebarGroupLabel = React.forwardRef<HTMLDivElement, React.ComponentProps<'div'> & { asChild?: boolean }>(
  ({ className, asChild = false, ...props }, ref) => {
    ...
    return (
      <Comp
        className={cn(
          `flex h-8 shrink-0 items-center rounded-md px-2 text-xs font-medium text-sidebar-foreground/70 outline-none ring-sidebar-ring transition-[margin,opa] duration-${SIDEBAR_ANIMATION_DURATION} ease-linear focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0`,
